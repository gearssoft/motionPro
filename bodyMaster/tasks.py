...
def detect_proc_pose(_depth_frame,_color_frame,pose_model):
    
    # mask_device = None
    pose_device = None

    # GPU 선택 숫자인지 확인후 int로 변환(GPU:0,GPU:1), CPU 선택시 문자열(CPU) 그대로 사용
    ...
    # 추론
    # mask_results = mask_model(frame, conf=mask_conf_thres,device=pose_device,verbose=False,classes=[0],iou = iou_threshold) 
    pose_results = pose_model.predict(frame,device=pose_device,verbose=False)
...
def server_task(depth_cam,pose_model,version_info,port,send_process_msg):
    ...
    def onTcpPacket(conn,_data, rinfo):
        ...
        if cmd == 0x10 :
            ...
            _packet = pack('<LBBBB',checkcode,cmd,*version_info)
...
def rendering_task(depth_cam,pose_model):
    ...
    while bStopRenderTask == False:
        ...
        if send_data_type == 'pose':
            _posePacket = detect_proc_pose(_depth_frame, frame,pose_model=pose_model)
        elif send_data_type == 'all':
        ...
        if _posePacket is not None:        
            ...
            for idx in range(detectionCount):
                ...
                if keypointCount == 17:
                    ...
                    left_eye = _keypoints[1]
                    right_eye = _keypoints[2]
                    ...
                    #draw eye
                    pygame.draw.rect(screen, white, pygame.Rect(int( left_eye[0] * screen_width), int(left_eye[1] * screen_height), 16, 16), 1)
                    pygame.draw.rect(screen, white, pygame.Rect(int( right_eye[0] * screen_width), int(right_eye[1] * screen_height), 16, 16), 1)
