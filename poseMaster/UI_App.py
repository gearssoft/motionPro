...
class ProcessManager:
    ...
    def start_process(self, self_button):
        ...
        port = self.port_entry.get()
        # 커맨드라인 명령어를 안전하게 구성
        command = f"python app.py --camera={camera} --port={port} --mode=server --debug_port={self.debug_port}"
        # command = f"app --camera={camera} --port={port} --mode=server --debug_port={self.debug_port}"
        self.save_settings()
...
# 메인 윈도우 생성 및 앱 실행
    ...
    def _auto_start():            
        ...
        progressList = [proMngUI1, proMngUI2]
        
        for _progree in progressList:
            _progree.start_process(_progree.start_button)
            
            while not bNextStep:
                if _progree.process.poll() is not None:
                    print(f'process{_progree.cam_index} is terminated')
                    _progree.start_button.config(state=tk.NORMAL)
                    _progree.output_text.insert(tk.END, f"\nProcess{_progree.cam_index} failed.\n")
                    break
                time.sleep(1)
        ...
        # while not bNextStep:
        #     if proMngUI1.process.poll() is not None:
        #         print('process1 is terminated')
        #         proMngUI1.start_button.config(state=tk.NORMAL)
        #         proMngUI1.output_text.insert(tk.END, "\nProcess1 stopped.\n")
